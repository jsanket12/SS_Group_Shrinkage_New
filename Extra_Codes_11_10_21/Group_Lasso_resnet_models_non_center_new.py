...
def SS_Group_Lasso_Node_conv3x3(in_planes, out_planes, lamb_mu, lamb_rho, stride=1, 
    ...
    return SS_Group_Lasso_Node_Conv2d_layer(in_planes, out_planes, lamb_mu, lamb_rho, kernel_size=3, stride=stride,
                     padding=1, bias=True, temp=temp, gamma_prior=gamma_prior, sigma_0=sigma_0)
...
class SS_Group_Lasso_Node_BasicBlock(nn.Module):
    ...
    def forward(self, x):
        out = self.silu(self.conv1(x)) # F.silu(self.bn1(self.conv1(x)))
...
class SS_Group_Lasso_Node_ResNet(nn.Module):
    def __init__(self, depth, num_classes=1000, temp = 0.5, gamma_prior = 0.0001, sigma_0 = 1, lamb_c = 4, lamb_d = 2):
        ...
        self.conv1 = SS_Group_Lasso_Node_Conv2d_layer(3, 16, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho, 
                                    kernel_size=3, padding=1, bias=True, temp=temp, gamma_prior=gamma_prior, sigma_0=sigma_0)
        self.silu = nn.SiLU(inplace=True)
        # self.bn1 = SS_Group_Lasso_VB_BatchNorm2d(16, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho, 
        ...
        for name, m in self.named_modules():
            if isinstance(m, SS_Group_Lasso_Node_BasicBlock):
                # GL_scale = torch.sqrt(torch.exp(m.conv1.tau_star_mu))
                # if m.conv1.transposed:
                #     GL_scale_w = GL_scale.expand(
                #                     m.conv1.in_channels, *m.conv1.kernel_size, m.conv1.out_channels // m.conv1.groups).permute(0,3,1,2)
                # else:
                #     GL_scale_w = GL_scale.expand( 
                #                     m.conv1.in_channels // m.conv1.groups, *m.conv1.kernel_size, m.conv1.out_channels).permute(3,0,1,2)
                nn.init.normal_(m.conv1.w_mu, mean=0, std=(np.sqrt(2 / (m.conv1.w_mu.shape[0] * np.prod(m.conv1.w_mu.shape[2:]))) * self.num_layers ** (-0.5))/np.sqrt(np.exp(1)))
                nn.init.constant_(m.conv1.v_mu, 0)
                # nn.init.constant_(m.conv1.w_mu, 0)
                nn.init.constant_(m.conv2.w_mu, 0)
                nn.init.constant_(m.conv2.v_mu, 0)
            elif isinstance(m, Group_Lasso_layer):
                nn.init.constant_(m.w_mu, 0)
                nn.init.constant_(m.v_mu, 0)
    ...
    def forward(self, x):
        ...
        x = self.silu(self.conv1(x)) # F.silu(self.bn1(self.conv1(x)))    # 32x32
...
def Group_Lasso_conv3x3(in_planes, out_planes, lamb_mu, lamb_rho, stride=1, sigma_0 = 1):
    ...
    return Group_Lasso_Conv2d_layer(in_planes, out_planes, lamb_mu, lamb_rho, kernel_size=3, stride=stride,
                     padding=1, bias=True, sigma_0=sigma_0)
...
class Group_Lasso_ResNet(nn.Module):
    ...
    def __init__(self, depth, num_classes=1000, sigma_0 = 1, lamb_c = 4, lamb_d = 2):
        ...
        #         m.bias_mu.data.zero_()
        for name, m in self.named_modules():
            if isinstance(m, Group_Lasso_BasicBlock):
                # GL_scale = torch.sqrt(torch.exp(m.conv1.tau_star_mu))
                # if m.conv1.transposed:
                #     GL_scale_w = GL_scale.expand(
                #                     m.conv1.in_channels, *m.conv1.kernel_size, m.conv1.out_channels // m.conv1.groups).permute(0,3,1,2)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        layers = []
        layers.append(block(self.inplanes, planes, self.lamb_mu, self.lamb_rho, stride, sigma_0=self.sigma_0))
