...
from Group_Lasso_Conv_layers import SS_Group_Lasso_Node_Conv2d_layer, Group_Lasso_Conv2d_layer
from Group_Lasso_linear_layers import SS_Group_Lasso_Node_layer, Group_Lasso_layer

# PATH = './results_lenet/mnist/SS_GL/'

parser = argparse.ArgumentParser(description='MNIST Lenet')

# Basic Setting
parser.add_argument('--seed', default=1, type = int, help = 'set seed')
# parser.add_argument('--results_path', default='./results_lenet_new/mnist/SS_GL/', type = str, help = 'base path for saving result')
...
parser.add_argument('--lamb_c', default = 4, type = float, help = 'lambda_c')
parser.add_argument('--lamb_d', default = 2, type = float, help = 'lambda_d')
...
class SFunc(nn.Module):
    """
        Our BNN
    """
    def __init__(self, hidden_dim1, hidden_dim2, target_dim, temp, gamma_prior, gamma_prior_star1, gamma_prior_star2, sigma_0, lamb_c, lamb_d):
        ...
        self.conv1 = SS_Group_Lasso_Node_Conv2d_layer(1, 20, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho,
                                                kernel_size=5, stride=1, temp=temp, gamma_prior=gamma_prior_star1, sigma_0=sigma_0)
        self.conv2 = SS_Group_Lasso_Node_Conv2d_layer(20, 50, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho,
                                                kernel_size=5, stride=1, temp=temp, gamma_prior=gamma_prior_star2, sigma_0=sigma_0)
        self.l1 = SS_Group_Lasso_Node_layer(4*4*50, hidden_dim1, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho,
                                                temp=temp, gamma_prior=gamma_prior_star1, sigma_0=sigma_0)
        self.l2 = SS_Group_Lasso_Node_layer(hidden_dim1, hidden_dim2, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho,
                                                temp=temp, gamma_prior=gamma_prior_star2, sigma_0=sigma_0)
        self.l4 = Group_Lasso_layer(hidden_dim2, target_dim, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho, sigma_0=sigma_0)

        # Horseshoe without spike-and-slab
        # self.conv1 = Group_Lasso_Conv2d_layer(1, 20, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho,
        #                                         kernel_size=5, stride=1, sigma_0=sigma_0)
        # self.conv2 = Group_Lasso_Conv2d_layer(20, 50, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho,
        #                                         kernel_size=5, stride=1, sigma_0=sigma_0)
        # self.l1 = Group_Lasso_layer(4*4*50, hidden_dim1, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho, sigma_0=sigma_0)
        # self.l2 = Group_Lasso_layer(hidden_dim1, hidden_dim2, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho, sigma_0=sigma_0)
        # self.l4 = Group_Lasso_layer(hidden_dim2, target_dim, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho, sigma_0=sigma_0)
...
class RunManager():
    ...
    def end_epoch(self,epoch,train_loss,train_accuracy,test_loss,test_accuracy,sparsity_conv1,
                    sparsity_conv2,sparsity_mlp1,sparsity_mlp2,edge_sparsity,learning_rate,batch_size):
...
def main():
    ...
    lamb_c = torch.as_tensor(args.lamb_c).to(device)
    lamb_d = torch.as_tensor(args.lamb_d).to(device)
    ...
    net = net = SFunc(hidden_dim1, hidden_dim2, target_dim, temp, gamma_prior, 
                        gamma_prior_star1, gamma_prior_star2, sigma_0, lamb_c, lamb_d).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            kl_train = net.conv1.kl+net.conv2.kl+net.l1.kl+net.l2.kl+net.l4.kl+net.kl_lambda
        ...
        print('Epoch {}, Train Loss: {}, Train Accuracy: {}, Test Loss: {}, Test Accuracy: {},, Sparsity Conv layer-1: {}, \
                Sparsity Conv layer-2: {},Sparsity MLP layer-1: {}, Sparsity MLP layer-2: {} Edge sparsity: {}'.format(
                            epoch, train_loss, train_accuracy, test_loss, test_accuracy, 
                            sparsity_conv1_val, sparsity_conv2_val, sparsity_mlp1_val, sparsity_mlp2_val, sparsity_overall))

        m.end_epoch(epoch, train_loss, train_accuracy,
                    test_loss, test_accuracy, sparsity_conv1_val, sparsity_conv2_val, sparsity_mlp1_val, sparsity_mlp2_val,
                    sparsity_overall, learning_rate, args.batch_train)

    print('Finished Training')
    writer.close()
    
    # m.save(PATH,'results_SS_Group_Lasso_Node_New_Lenet_Caffe_MNIST_silu_0.001_1024_1200')

    # torch.save(net.state_dict(), PATH + 'SS_Group_Lasso_Node_New_Lenet_Caffe_MNIST_model_silu_0.001_1024_1200.pt')
        
    # plt.plot(range(num_epochs), train_Loss, 'b', label='Train')                
    # plt.plot(range(num_epochs), test_Loss, 'orange', label='Test')
    # plt.title('SS_Group_Lasso_Node: Train-Test Loss')
    # plt.xlabel('Epochs')
    # plt.ylabel('Loss')
    # # plt.ylim([0.55, 1.0])
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Group_Lasso_Node_New_Lenet_Caffe_MNIST_loss_silu_0.001_1024_1200.png"),dpi=300)
    # plt.close()

    # plt.plot(range(num_epochs), train_Accuracy, 'b', label='Train')
    # plt.plot(range(num_epochs), test_Accuracy, 'orange', label='Test')
    # plt.title('SS_Group_Lasso_Node: Train-Test Accuracy')
    # plt.xlabel('Epochs')
    # plt.ylabel('Accuracy')
    # # plt.ylim([0.55, 1.0])
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Group_Lasso_Node_New_Lenet_Caffe_MNIST_accuracy_silu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
    ...
    # plt.title('SS_Group_Lasso_Node: Node Sparsity')
    # plt.xlabel('Epochs')
    # plt.ylabel('Sparsity')
    # # plt.ylim([0.65, 1.0])
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Group_Lasso_Node_New_Lenet_Caffe_MNIST_sparsity_node_silu_0.001_1024_1200.png"),dpi=300)
    # plt.close()

    # plt.plot(range(num_epochs), Edge_sparsity, 'g')
    # plt.title('SS_Group_Lasso_Node: Edge Sparsity')
    # plt.xlabel('Epochs')
    # plt.ylabel('Sparsity')
    # # plt.ylim([0.65, 1.0])
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Group_Lasso_Node_New_Lenet_Caffe_MNIST_sparsity_edge_silu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
