...
class SS_GHS_VB_BatchNorm(SS_GHS_VB_NormBase):
    ...
    def forward(self, input_dict):
        ...
        if self.affine:
            ...
            weight = z * GHS_scale * (self.weight_mu + weight_sigma * weight_epsilon)

            bias_sigma = torch.log1p(torch.exp(self.bias_rho))
            bias_epsilon = torch.zeros_like(self.bias_mu).normal_()
            bias = z * GHS_scale * (self.bias_mu + bias_sigma * bias_epsilon)

            if self.training:  
                gamma = sigmoid(self.theta)
                self.kl_sig_contri = torch.sum(gamma)
                ...
                kl_a_tau = gamma * (-torch.log(self.tau_1) + torch.exp(self.tau_a_mu + 0.5*sigma_a_tau**2)/self.tau_1 - \
                            0.5*(self.tau_a_mu + 2*torch.log(sigma_a_tau) + 1.69315))

                kl_b_tau = gamma * (torch.exp(0.5*sigma_b_tau**2 - self.tau_b_mu) - \
            ...
            self.training or not self.track_running_stats,
            exponential_average_factor, self.eps), 1:Global_scale}
        else:
            weight = None
            bias = None
            return {0:F.batch_norm(
...
class GHS_VB_BatchNorm(GHS_VB_NormBase):
    ...
    def forward(self, input_dict):
        ...
        if self.affine:
            ...
            weight_sigma = torch.log1p(torch.exp(self.weight_rho))    
            weight_epsilon = torch.zeros_like(self.weight_mu).normal_()
            weight = GHS_scale * (self.weight_mu + weight_sigma * weight_epsilon)

            bias_sigma = torch.log1p(torch.exp(self.bias_rho))
            bias_epsilon = torch.zeros_like(self.bias_mu).normal_()
            bias = GHS_scale * (self.bias_mu + bias_sigma * bias_epsilon)
