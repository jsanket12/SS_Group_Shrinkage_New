...
import adabound
from Gauss_resnet_models import SSGauss_Edge_ResNet, SSGauss_Edge_Wide_ResNet
from Gauss_layers_new import SSGauss_Edge_layer
from Gauss_Conv_layers_new import SSGauss_Edge_Conv2d_layer
from Gauss_BN_layers import SSGauss_VB_BatchNorm2d

parser = argparse.ArgumentParser(description='Cifar10 ResNet')

# Basic Setting
parser.add_argument('--seed', default=1, type = int, help = 'set seed')
parser.add_argument('--results_path', default='./results_resnet/cifar10/SS_Gauss_Edge/', type = str, help = 'base path for saving result')
...
def main():
    ...
    net = SSGauss_Edge_ResNet(depth=args.depth, num_classes=target_dim, temp = temp, gamma_prior = gamma_prior, sigma_0 = sigma_0).to(device)
    ...
    for module in net.modules():
        if isinstance(module, (SSGauss_Edge_Conv2d_layer,SSGauss_Edge_layer)):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            kl_train = 0.
            for module in net.modules():
                if isinstance(module, (SSGauss_Edge_Conv2d_layer,SSGauss_VB_BatchNorm2d,SSGauss_Edge_layer)):
    ...
    m.save(PATH,'results_SS_Gaussian_Edge_Resnet_CIFAR10_silu_lr_decay_'+str(args.batch_train)) 

    torch.save(net.state_dict(), PATH + 'SS_Gaussian_Edge_Resnet_CIFAR10_model_silu_lr_decay_'+ str(args.batch_train) + '.pt')
    ...
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,'SS_Gaussian_Edge_Resnet_CIFAR10_loss_silu_lr_decay_'+str(args.batch_train)+'.png'),dpi=300)
    plt.close()
    ...
    plt.legend(loc='upper right', frameon=False)
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,'SS_Gaussian_Edge_Resnet_CIFAR10_accuracy_silu_lr_decay_'+str(args.batch_train)+'.png'),dpi=300)
    plt.close()
    ...
    plt.legend(loc='upper right', frameon=False)
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,'SS_Gaussian_Edge_Resnet_CIFAR10_sparsity_edge_silu_lr_decay_'+str(args.batch_train)+'.png'),dpi=300)
    plt.close()
