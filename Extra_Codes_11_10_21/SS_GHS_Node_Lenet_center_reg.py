...
from GHS_linear_layers_center_reg import SS_GHS_Node_layer, GHS_layer
...
parser.add_argument('--results_path', default='./results_lenet_new/mnist/SS_GHS/', type = str, help = 'base path for saving result')
...
class SFunc(nn.Module):
    """
        Our BNN
    """
    def __init__(self, hidden_dim1, hidden_dim2, target_dim, temp, gamma_prior, gamma_prior_star1, 
        ...
        # Spike-and-slab node selection with group horseshoe
        self.conv1 = SS_GHS_Node_Conv2d_layer(1, 20, sig_a_mu = self.sig_a_mu, sig_a_rho = self.sig_a_rho, 
                                                sig_b_mu = self.sig_b_mu, sig_b_rho = self.sig_b_rho,
                                                kernel_size=5, stride=1, temp=temp, gamma_prior=gamma_prior_star1, sigma_0=sigma_0, tau_1 = tau_1)
        self.conv2 = SS_GHS_Node_Conv2d_layer(20, 50, sig_a_mu = self.sig_a_mu, sig_a_rho = self.sig_a_rho, 
                                                sig_b_mu = self.sig_b_mu, sig_b_rho = self.sig_b_rho,
                                                kernel_size=5, stride=1, temp=temp, gamma_prior=gamma_prior_star2, sigma_0=sigma_0, tau_1 = tau_1)
        self.l1 = SS_GHS_Node_layer(4*4*50, hidden_dim1, sig_a_mu = self.sig_a_mu, sig_a_rho = self.sig_a_rho, 
                                                sig_b_mu = self.sig_b_mu, sig_b_rho = self.sig_b_rho, 
                                                temp=temp, gamma_prior=gamma_prior_star1, sigma_0=sigma_0, tau_1 = tau_1)
        self.l2 = SS_GHS_Node_layer(hidden_dim1, hidden_dim2, sig_a_mu = self.sig_a_mu, sig_a_rho = self.sig_a_rho, 
                                                sig_b_mu = self.sig_b_mu, sig_b_rho = self.sig_b_rho, 
                                                temp=temp, gamma_prior=gamma_prior_star2, sigma_0=sigma_0, tau_1 = tau_1)
        ...
        # Group horseshoe without spike-and-slab
        # self.conv1 = GHS_Conv2d_layer(1, 20, sig_a_mu = self.sig_a_mu, sig_a_rho = self.sig_a_rho, 
        #                                         sig_b_mu = self.sig_b_mu, sig_b_rho = self.sig_b_rho,
        #                                         kernel_size=5, stride=1, sigma_0=sigma_0, tau_1 = tau_1)
    ...
    def forward(self, X):
        ...
        output = self.conv1(x_dict)
        output = F.silu(F.max_pool2d(output[0], 2))
        output = self.conv2({0:output, 1:Global_scale, 2:self.c_reg})
        output = F.silu(F.max_pool2d(output[0], 2))
        output = self.l1({0:output.reshape(-1,4*4*50), 1:Global_scale, 2:self.c_reg})
...
class RunManager():
    ...
    def end_epoch(self,epoch,train_loss,train_accuracy,test_loss,test_accuracy,sparsity_conv1,sparsity_conv2,sparsity_mlp1,sparsity_mlp2,edge_sparsity,learning_rate,batch_size):
...
def main():
    ...
    net = SFunc(hidden_dim1, hidden_dim2, target_dim, temp, gamma_prior, gamma_prior_star1, gamma_prior_star2, 
                        sigma_0, tau_0, tau_1, c_a = c_a, c_b = c_b, c_reg = c_reg).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            kl_train = net.conv1.kl+net.conv2.kl+net.l1.kl+net.l2.kl+net.l4.kl+net.kl_sig_and_c
    ...
    m.save(PATH,'results_SS_Group_Horseshoe_Node_center_reg_Lenet_Caffe_MNIST_silu_0.001_1024_1200')

    torch.save(net.state_dict(), PATH + 'SS_Group_Horseshoe_Node_center_reg_Lenet_Caffe_MNIST_model_silu_0.001_1024_1200.pt')
        
    plt.plot(range(num_epochs), train_Loss, 'b', label='Train')                
    plt.plot(range(num_epochs), test_Loss, 'orange', label='Test')
    plt.title('SS_Group_Horseshoe_Node: Train-Test Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    # plt.ylim([0.55, 1.0])
    plt.legend(loc='upper right', frameon=False)
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,"SS_Group_Horseshoe_Node_center_reg_Lenet_Caffe_MNIST_loss_silu_0.001_1024_1200.png"),dpi=300)
    plt.close()

    plt.plot(range(num_epochs), train_Accuracy, 'b', label='Train')
    plt.plot(range(num_epochs), test_Accuracy, 'orange', label='Test')
    plt.title('SS_Group_Horseshoe_Node: Train-Test Accuracy')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    # plt.ylim([0.55, 1.0])
    plt.legend(loc='upper right', frameon=False)
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,"SS_Group_Horseshoe_Node_center_reg_Lenet_Caffe_MNIST_accuracy_silu_0.001_1024_1200.png"),dpi=300)
    plt.close()
    ...
    plt.title('SS_Group_Horseshoe_Node: Node Sparsity')
    plt.xlabel('Epochs')
    plt.ylabel('Sparsity')
    # plt.ylim([0.65, 1.0])
    plt.legend(loc='upper right', frameon=False)
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,"SS_Group_Horseshoe_Node_center_reg_Lenet_Caffe_MNIST_sparsity_node_silu_0.001_1024_1200.png"),dpi=300)
    plt.close()

    plt.plot(range(num_epochs), Edge_sparsity, 'g')
    plt.title('SS_Group_Horseshoe_Node: Edge Sparsity')
    plt.xlabel('Epochs')
    plt.ylabel('Sparsity')
    # plt.ylim([0.65, 1.0])
    plt.legend(loc='upper right', frameon=False)
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,"SS_Group_Horseshoe_Node_center_reg_Lenet_Caffe_MNIST_sparsity_edge_silu_0.001_1024_1200.png"),dpi=300)
    plt.close()
