...
from Gauss_resnet_models_new import SSGauss_Node_ResNet
from Gauss_linear_layers import Gauss_layer
from Gauss_Conv_layers_new import SSGauss_Node_Conv2d_layer
...
parser.add_argument('--results_path', default='./results_resnet_new/cifar10/SS_Gauss_Node_new/', type = str, help = 'base path for saving result')
...
class RunManager():
    ...
    def end_epoch(self,epoch,train_loss,train_accuracy,test_loss,
                    test_accuracy,edge_sparsity,param_pruned,
                    flops_ratio,flops_pruned,learning_rate,batch_size):
...
def main():
    ...
    net = SSGauss_Node_ResNet(depth=args.depth, num_classes=target_dim, temp = temp, gamma_prior = gamma_prior, sigma_0 = sigma_0).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            # loss = (nll_train/float(num_MC_train)) + kl_train
        ...
        with torch.no_grad():
            ...
            if test_accuracy > best_accuracy:
                ...
                # torch.save(net.state_dict(), PATH + 'SS_Gaussian_Node_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_seed_'+str(args.seed) + '_best_model.pt')
    ...
    # net.load_state_dict(torch.load(PATH + 'SS_Gaussian_Node_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_seed_'+str(args.seed)+'_best_model.pt'))
    ...
    #         if test_accuracy > best_accuracy:
    #             best_accuracy = test_accuracy
    #             torch.save(net.state_dict(), PATH + 'SS_Gaussian_Node_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_seed_'+str(args.seed)+'_final_model.pt')
    ...
    # m.save(PATH,'results_SS_Gaussian_Node_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_seed_'+str(args.seed)) 

    # # torch.save(net.state_dict(), PATH + 'SS_Gaussian_Node_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0) + '_model.pt')
    ...
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,'SS_Gaussian_Node_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_loss.png'),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,'SS_Gaussian_Node_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_accuracy.png'),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,'SS_Gaussian_Node_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_edge_sparsity.png'),dpi=300)
    # plt.close()
