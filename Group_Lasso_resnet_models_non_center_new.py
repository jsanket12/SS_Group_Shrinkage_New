...
from Group_Lasso_Conv_layers_non_center_new import SS_Group_Lasso_Node_Conv2d_layer, Group_Lasso_Conv2d_layer
...
def SS_Group_Lasso_Node_conv3x3(in_planes, out_planes, lamb_mu, lamb_rho, fine_tune, stride=1, 
                        temp = 0.5, gamma_prior = 0.0001, sigma_0 = 1):
    "3x3 convolution with padding"
    
    return SS_Group_Lasso_Node_Conv2d_layer(in_planes, out_planes, lamb_mu, lamb_rho, fine_tune, kernel_size=3, stride=stride,
                     padding=1, bias=False, temp=temp, gamma_prior=gamma_prior, sigma_0=sigma_0)

class SS_Group_Lasso_Node_BasicBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, lamb_mu, lamb_rho, fine_tune, stride=1, downsample=None, 
                temp = 0.5, gamma_prior = 0.0001, sigma_0 = 1):
        super().__init__()
        self.conv1 = SS_Group_Lasso_Node_conv3x3(inplanes, planes, lamb_mu, lamb_rho, fine_tune, stride, temp=temp, gamma_prior=gamma_prior, sigma_0=sigma_0)
        self.bn1 = Group_Lasso_VB_BatchNorm2d(planes, lamb_mu, lamb_rho, sigma_0=sigma_0)
        self.conv2 = SS_Group_Lasso_Node_conv3x3(planes, planes, lamb_mu, lamb_rho, fine_tune, temp=temp, gamma_prior=gamma_prior, sigma_0=sigma_0)
...
class SS_Group_Lasso_Node_ResNet(nn.Module):
    def __init__(self, depth, num_classes=1000, temp = 0.5, gamma_prior = 0.0001, sigma_0 = 1, lamb_c = 4, lamb_d = 2):
        ...
        self.inplanes = 16
        self.conv1 = SS_Group_Lasso_Node_Conv2d_layer(3, 16, lamb_mu = self.lamb_mu, lamb_rho = self.lamb_rho, fine_tune = self.fine_tune,
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                SS_Group_Lasso_Node_Conv2d_layer(self.inplanes, planes * block.expansion, self.lamb_mu, self.lamb_rho, self.fine_tune, 
        ...
        layers = []
        layers.append(block(self.inplanes, planes, self.lamb_mu, self.lamb_rho, self.fine_tune,
