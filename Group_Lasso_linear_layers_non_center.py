...
class SS_Group_Lasso_Node_and_Edge_layer(nn.Module):
    ...
    def forward(self, input):
        ...
        self.w = w_z * GL_scale.expand(self.input_dim, self.output_dim) *(self.w_mu + sigma_w * epsilon_w)
        ...
        if self.training:
            ...
            kl_w = w_gamma_star * w_gamma * (torch.log(self.sigma_0) - torch.log(sigma_w) - 0.5 + \
            ...
            if self.v_mu is not None:
                ...
                kl_v = v_gamma_star * v_gamma * (torch.log(self.sigma_0) - torch.log(sigma_v) - 0.5 + \
...
class Group_Lasso_layer(nn.Module):
    ...
    def forward(self, input):
        ...
        sigma_w = torch.log1p(torch.exp(self.w_rho))
        epsilon_w = torch.zeros_like(self.w_mu).normal_()
        self.w = GL_scale.expand(self.input_dim, self.output_dim) *(self.w_mu + sigma_w * epsilon_w)
