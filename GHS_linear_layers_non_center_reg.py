...
class SS_GHS_Node_and_Edge_layer(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        self.w = w_z * GHS_scale_reg.expand(self.input_dim, self.output_dim) *(self.w_mu + sigma_w * epsilon_w)
...
class SS_GHS_Node_layer(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        if self.v_mu is not None:
            epsilon_v = torch.zeros_like(self.v_mu).normal_()
            sigma_v = torch.log1p(torch.exp(self.v_rho))
            self.v = z * GHS_scale_reg * (self.v_mu + sigma_v * epsilon_v)
        else:
            self.v = None 
...
class GHS_layer(nn.Module):
    def __init__(self, input_dim, output_dim, bias=True, sigma_0=1, tau_1 =1):
        super().__init__()
        # set input and output dimensions
        self.input_dim = input_dim
        self.output_dim = output_dim
    ...
    def forward(self, input_dict):
        ...
        GHS_scale = Global_scale * \
                        torch.exp(0.5*(self.tau_a_mu+self.tau_b_mu) + sigma_tau * epsilon_tau)
        GHS_scale_reg = (c_reg * GHS_scale)/torch.sqrt(c_reg**2 + GHS_scale**2)
        self.w = GHS_scale_reg.expand(self.input_dim, self.output_dim) *(self.w_mu + sigma_w * epsilon_w)
        if self.v_mu is not None:
            epsilon_v = torch.zeros_like(self.v_mu).normal_()
            sigma_v = torch.log1p(torch.exp(self.v_rho))
