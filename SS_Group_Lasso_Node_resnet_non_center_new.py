...
from Group_Lasso_resnet_models_non_center_new import SS_Group_Lasso_Node_ResNet
from Group_Lasso_linear_layers_non_center import Group_Lasso_layer
from Group_Lasso_Conv_layers_non_center_new import SS_Group_Lasso_Node_Conv2d_layer
...
parser.add_argument('--results_path', default='./results_resnet_new/cifar10/SS_GL_new/', type = str, help = 'base path for saving result')
...
def main():
    ...
    net = SS_Group_Lasso_Node_ResNet(depth=args.depth, num_classes=target_dim, temp = temp, gamma_prior = gamma_prior, sigma_0 = sigma_0, 
                            lamb_c = lamb_c, lamb_d = lamb_d).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            for module in net.modules():
                if isinstance(module, (SS_Group_Lasso_Node_Conv2d_layer,Group_Lasso_VB_BatchNorm2d,Group_Lasso_layer)):
                    kl_train += module.kl.div(NTrain)
            kl_train += net.kl_lambda.div(NTrain).squeeze()
        ...
        with torch.no_grad():
                ...
                for it in range(num_MC_test):       
                    ...
                    for name, module in net.named_modules():
                        if 'downsample' not in name:
                            if isinstance(module, (SS_Group_Lasso_Node_Conv2d_layer)): 
                                ...
                                if module.v is not None:
                                    ...
                                    flops_overall += out_prune_channels*(in_prune_channels*module.kernel_size[0]*module.kernel_size[1])* \
                                                    (np.floor((module.input_size[2]+2*module.padding[0]-module.dilation[0]*(module.kernel_size[0]-1)-1)/module.stride[0]+1))**2/module.groups                        
                            elif isinstance(module, (Group_Lasso_layer)):
                                if module.v is not None:
            ...
            if test_accuracy > best_accuracy:
                ...
                torch.save(net.state_dict(), PATH + 'SS_Group_Lasso_Node_non_center_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_seed_'+str(args.seed) + '_best_model.pt')
    ...
    net.load_state_dict(torch.load(PATH + 'SS_Group_Lasso_Node_non_center_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_seed_'+str(args.seed) + '_best_model.pt'))
    ...
    for epoch_fine in range(args.fine_tune_nepoch):
        ...
        with torch.no_grad():
            ...
            if test_accuracy > best_accuracy:
                best_accuracy = test_accuracy
                torch.save(net.state_dict(), PATH + 'SS_Group_Lasso_Node_non_center_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0) +'_seed_'+str(args.seed) + '_final_model.pt')
    ...
    m.save(PATH,'results_SS_Group_Lasso_Node_non_center_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_seed_'+str(args.seed)) 

    # torch.save(net.state_dict(), PATH + 'SS_Group_Lasso_Node_non_center_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0) + '_model.pt')
    ...
    plt.title('SS_Group_Lasso_Node_non_center: Train-Test Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    # plt.ylim([0.55, 1.0])
    plt.legend(loc='upper right', frameon=False)
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,'SS_Group_Lasso_Node_non_center_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_loss.png'),dpi=300)
    plt.close()

    plt.plot(range(num_epochs), train_Accuracy, 'b', label='Train')
    plt.plot(range(num_epochs), test_Accuracy, 'orange', label='Test')
    plt.title('SS_Group_Lasso_Node_non_center: Train-Test Accuracy')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    # plt.ylim([0.55, 1.0])
    plt.legend(loc='upper right', frameon=False)
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,'SS_Group_Lasso_Node_non_center_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_accuracy.png'),dpi=300)
    plt.close()

    plt.plot(range(num_epochs), Edge_sparsity, 'g')
    plt.title('SS_Group_Lasso_Node_non_center: Edge Sparsity')
    plt.xlabel('Epochs')
    plt.ylabel('Sparsity')
    # plt.ylim([0.65, 1.0])
    plt.legend(loc='upper right', frameon=False)
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,'SS_Group_Lasso_Node_non_center_fine_tune_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_edge_sparsity.png'),dpi=300)
    plt.close()
