...
class SS_GHS_VB_BatchNorm(SS_GHS_VB_NormBase):
    ...
    def __init__(self, num_features, sig_a_mu, sig_a_rho, sig_b_mu, sig_b_rho, eps=1e-5, momentum=0.1, affine=True,
    ...
    def forward(self, input_dict):
        ...
        if self.affine:
            ...
            if self.training:
                ...
                kl_weight = gamma * (torch.log(self.sigma_0) + torch.log(c_reg) - torch.log(weight_sigma) - 0.5 + \
                            0.5*(self.tau_a_mu + self.tau_b_mu + self.sig_a_mu + self.sig_b_mu) - 0.5*torch.log(c_reg**2  + GHS_scale**2) + \
                            0.5*((weight_sigma**2+self.weight_mu**2)/self.sigma_0**2)* \
                            (torch.exp(-(self.tau_a_mu + self.tau_b_mu + self.sig_a_mu + self.sig_b_mu) + \
                                0.5*(sigma_a_tau**2 + sigma_b_tau**2 + sig_a_sigma**2 + sig_b_sigma**2)) + 1/(c_reg**2))) 
            
                kl_bias = gamma * (torch.log(self.sigma_0) + torch.log(c_reg) - torch.log(bias_sigma) - 0.5 + \
                            0.5*(self.tau_a_mu + self.tau_b_mu + self.sig_a_mu + self.sig_b_mu) - 0.5*torch.log(c_reg**2  + GHS_scale**2) + \
    ...
    def forward(self, input_dict):
        ...
        if self.affine:
            ...
            if self.training:
                ...
                kl_weight = torch.log(self.sigma_0) + torch.log(c_reg) - torch.log(weight_sigma) - 0.5 + \
                            0.5*(self.tau_a_mu + self.tau_b_mu + self.sig_a_mu + self.sig_b_mu) - 0.5*torch.log(c_reg**2  + GHS_scale**2) + \
                            0.5*((weight_sigma**2+self.weight_mu**2)/self.sigma_0**2)* \
                            (torch.exp(-(self.tau_a_mu + self.tau_b_mu + self.sig_a_mu + self.sig_b_mu) + \
                                0.5*(sigma_a_tau**2 + sigma_b_tau**2 + sig_a_sigma**2 + sig_b_sigma**2)) + 1/(c_reg**2)) 
            
                kl_bias = torch.log(self.sigma_0) + torch.log(c_reg) - torch.log(bias_sigma) - 0.5 + \
                            0.5*(self.tau_a_mu + self.tau_b_mu + self.sig_a_mu + self.sig_b_mu) - 0.5*torch.log(c_reg**2  + GHS_scale**2) + \
                            0.5*((bias_sigma**2+self.bias_mu**2)/self.sigma_0**2)* \
                            (torch.exp(-(self.tau_a_mu + self.tau_b_mu + self.sig_a_mu + self.sig_b_mu) + \
                                0.5*(sigma_a_tau**2 + sigma_b_tau**2 + sig_a_sigma**2 + sig_b_sigma**2)) + 1/(c_reg**2))
