...
class SS_GHS_Node_and_Edge_layer(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        if self.training:
            ...
            GHS_scale_w = GHS_scale.expand(self.input_dim, self.output_dim)           
            ...
            kl_w = w_gamma_star * w_gamma * (torch.log(self.sigma_0) + torch.log(c_reg) - torch.log(sigma_w) - 0.5 + \
                    0.5*(w_tau_a_mu + w_tau_b_mu + self.sig_a_mu + self.sig_b_mu) - 0.5*torch.log(c_reg**2  + GHS_scale_w**2) + \
            ...
            if self.v_mu is not None:
                ...
                kl_v = v_gamma_star * v_gamma * (torch.log(self.sigma_0) + torch.log(c_reg) - torch.log(sigma_v) - 0.5 + \
                        0.5*(v_tau_a_mu + v_tau_b_mu + self.sig_a_mu + self.sig_b_mu) - 0.5*torch.log(c_reg**2  + GHS_scale_v**2) + \
                ...
                self.kl = torch.sum(kl_w_gamma) + torch.sum(kl_gamma_star) + torch.sum(kl_w) + torch.sum(kl_a_tau) + torch.sum(kl_b_tau) 
    ...
    def forward(self, input_dict):
        ...
        if self.training:
            ...
            if self.v_mu is not None:
                ...
                self.kl = torch.sum(kl_gamma) + torch.sum(kl_w) + torch.sum(kl_v) + torch.sum(kl_a_tau) + torch.sum(kl_b_tau)
            else:
                self.kl = torch.sum(kl_gamma) + torch.sum(kl_w) + torch.sum(kl_a_tau) + torch.sum(kl_b_tau)              
    ...
    def forward(self, input_dict):
        ...
        if self.training:
            ...
            if self.v_mu is not None:
                ...
                self.kl = torch.sum(kl_w_gamma) + torch.sum(kl_w) + torch.sum(kl_a_tau) + torch.sum(kl_b_tau)               
