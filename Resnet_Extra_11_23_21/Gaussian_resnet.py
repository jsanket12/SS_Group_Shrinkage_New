...
from Gauss_resnet_models import Gauss_ResNet
...
parser.add_argument('--results_path', default='./results_resnet_new/cifar10/Gauss/', type = str, help = 'base path for saving result')
...
def main():
    ...
    net = Gauss_ResNet(depth=args.depth, num_classes=target_dim, sigma_0 = sigma_0).to(device)
    ...
    m.save(PATH,'results_Gaussian_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)) 

    torch.save(net.state_dict(), PATH + 'Gaussian_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_model.pt')
    ...
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,'Gaussian_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_loss.png'),dpi=300)
    plt.close()
    ...
    plt.legend(loc='upper right', frameon=False)
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,'Gaussian_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_accuracy.png'),dpi=300)
    plt.close()
