...
from Group_Lasso_resnet_models_new import SS_Group_Lasso_Node_ResNet
from Group_Lasso_linear_layers import Group_Lasso_layer
from Group_Lasso_Conv_layers import SS_Group_Lasso_Node_Conv2d_layer
from Group_Lasso_BN_layers import SS_Group_Lasso_VB_BatchNorm2d

parser = argparse.ArgumentParser(description='Cifar10 ResNet')

# Basic Setting
parser.add_argument('--seed', default=1, type = int, help = 'set seed')
parser.add_argument('--results_path', default='./results_resnet_new/cifar10/SS_GL/', type = str, help = 'base path for saving result')
...
def main():
    ...
    #     if isinstance(module, (SS_Group_Lasso_Node_Conv2d_layer,Group_Lasso_layer)):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            kl_train = 0.
            for module in net.modules():
                if isinstance(module, (SS_Group_Lasso_Node_Conv2d_layer,SS_Group_Lasso_VB_BatchNorm2d,Group_Lasso_layer)):
            ...
            for cnt, (images, labels) in enumerate(test_loader):
                ...
                for it in range(num_MC_test):       
                    ...
                    for name, module in net.named_modules():
                        if isinstance(module, (SS_Group_Lasso_Node_Conv2d_layer)):
                            if 'downsample' not in name:
                                ...
                                if module.v is not None:
                                    param_overall += (in_prune_channels*module.kernel_size[0]*module.kernel_size[1]+1)*out_prune_channels
                                    ...
                                    param_overall += (in_prune_channels*module.kernel_size[0]*module.kernel_size[1])*out_prune_channels
                                    flops_overall += out_prune_channels*(in_prune_channels*module.kernel_size[0]*module.kernel_size[1])* \
                                                    (np.floor((module.input_size[2]+2*module.padding[0]-module.dilation[0]*(module.kernel_size[0]-1)-1)/module.stride[0]+1))**2/module.groups
                                in_prune_channels = out_prune_channels
                        ...
                        # elif isinstance(module, (SS_Group_Lasso_VB_BatchNorm2d)):
                        #     print(out_prune_channels == (module.z != 0).sum())
                        #     in_prune_channels = (module.z != 0).sum()
    ...
    # m.save(PATH,'results_SS_Group_Lasso_Node_Resnet_CIFAR10_silu_lr_decay_'+str(args.batch_train)) 

    # torch.save(net.state_dict(), PATH + 'SS_Group_Lasso_Node_Resnet_CIFAR10_model_silu_lr_decay_'+ str(args.batch_train) + '.pt')
    ...
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,'SS_Group_Lasso_Node_Resnet_CIFAR10_loss_silu_lr_decay_'+str(args.batch_train)+'.png'),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,'SS_Group_Lasso_Node_Resnet_CIFAR10_accuracy_silu_lr_decay_'+str(args.batch_train)+'.png'),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,'SS_Group_Lasso_Node_Resnet_CIFAR10_sparsity_edge_silu_lr_decay_'+str(args.batch_train)+'.png'),dpi=300)
    # plt.close()
