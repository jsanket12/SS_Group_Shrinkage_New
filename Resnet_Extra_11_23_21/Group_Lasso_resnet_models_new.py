...
from Group_Lasso_BN_layers import SS_Group_Lasso_VB_BatchNorm2d, Group_Lasso_VB_BatchNorm2d
...
class SS_Group_Lasso_Node_BasicBlock(nn.Module):
    ...
    def forward(self, x):
        out = F.silu(self.conv1(x)) # F.silu(self.bn1(self.conv1(x)))
...
class SS_Group_Lasso_Node_ResNet(nn.Module):
    def __init__(self, depth, num_classes=1000, temp = 0.5, gamma_prior = 0.0001, sigma_0 = 1, lamb_c = 4, lamb_d = 2):
        ...
        for m in self.modules():
            if isinstance(m, SS_Group_Lasso_Node_BasicBlock):
                nn.init.normal_(m.conv1.w_mu, mean=0, std=np.sqrt(2 / (m.conv1.w_mu.shape[0] * np.prod(m.conv1.w_mu.shape[2:]))) * self.num_layers ** (-0.5))
    ...
    def forward(self, x):
        ...
        x = F.silu(self.conv1(x)) # F.silu(self.bn1(self.conv1(x)))    # 32x32
