...
##########################################################################################################################################
#### Spike-and-slab group lasso batchnorm Layer
class SS_Group_Lasso_VB_NormBase(torch.nn.Module):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
        ...
        if self.affine:
            ...
            self.tau_star_mu = nn.Parameter(torch.Tensor(num_features))
            self.tau_star_rho = nn.Parameter(torch.Tensor(num_features))
            ...
            self.register_parameter('tau_star_mu', None)
            self.register_parameter('tau_star_rho', None)
...
class SS_Group_Lasso_VB_BatchNorm(SS_Group_Lasso_VB_NormBase):

    def __init__(self, num_features, lamb_mu, lamb_rho, eps=1e-5, momentum=0.1, affine=True,
    ...
    def forward(self, input):
        ...
        if self.affine:
            ...
            if self.training:  
                ...
                kl_weight = gamma * (torch.log(self.sigma_0) - torch.log(weight_sigma) + 0.5*self.tau_star_mu - 0.5 + \
                    0.5*((weight_sigma**2+self.weight_mu**2)/self.sigma_0**2)*torch.exp(-self.tau_star_mu + (sigma_tau_star**2/2)))
            
                kl_bias = gamma * (torch.log(self.sigma_0) - torch.log(bias_sigma) + 0.5*self.tau_star_mu - 0.5 + \
                    0.5*((bias_sigma**2+self.bias_mu**2)/self.sigma_0**2)*torch.exp(-self.tau_star_mu + (sigma_tau_star**2/2)))
                
                kl_tau = -torch.tensor(2)*(self.lamb_mu+self.tau_star_mu) + \
                    0.5*torch.exp(self.lamb_mu + (lamb_sigma**2/2))*torch.exp(self.tau_star_mu + (sigma_tau_star**2/2)) -\
                    torch.log(sigma_tau_star) -0.032645
                
                self.kl = torch.sum(kl_gamma) + torch.sum(kl_weight) + torch.sum(kl_bias) + torch.sum(kl_tau)
...
class SS_Group_Lasso_VB_BatchNorm2d(SS_Group_Lasso_VB_BatchNorm):
    def _check_input_dim(self, input):
        if input.dim() != 4:
            raise ValueError('expected 4D input (got {}D input)'
                             .format(input.dim()))

class SS_Group_Lasso_VB_BatchNorm1d(SS_Group_Lasso_VB_BatchNorm):
    def _check_input_dim(self, input):
...
##########################################################################################################################################
#### Group lasso batchnorm Layer
class Group_Lasso_VB_NormBase(torch.nn.Module):
    """Common base of _InstanceNorm and _BatchNorm"""
    _version = 2
    __constants__ = ['track_running_stats', 'momentum', 'eps', 'weight', 'bias',
                     'running_mean', 'running_var', 'num_batches_tracked',
...
class Group_Lasso_VB_BatchNorm(Group_Lasso_VB_NormBase):
    ...
    def forward(self, input):
        ...
        if self.affine:
            ...
            if self.training:
                ...
                kl_weight = torch.log(self.sigma_0) - torch.log(weight_sigma) + 0.5*self.tau_star_mu - 0.5 + \
                    0.5*((weight_sigma**2+self.weight_mu**2)/self.sigma_0**2)*torch.exp(-self.tau_star_mu + (sigma_tau_star**2/2))
            
                kl_bias = torch.log(self.sigma_0) - torch.log(bias_sigma) + 0.5*self.tau_star_mu - 0.5 + \
                    0.5*((bias_sigma**2+self.bias_mu**2)/self.sigma_0**2)*torch.exp(-self.tau_star_mu + (sigma_tau_star**2/2))
                ...
                self.kl = torch.sum(kl_weight) + torch.sum(kl_bias) + torch.sum(kl_tau)
...
class Group_Lasso_VB_BatchNorm2d(Group_Lasso_VB_BatchNorm):
    def _check_input_dim(self, input):
        if input.dim() != 4:
            raise ValueError('expected 4D input (got {}D input)'
                             .format(input.dim()))

class Group_Lasso_VB_BatchNorm1d(Group_Lasso_VB_BatchNorm):
    def _check_input_dim(self, input):
        if input.dim() != 2 and input.dim() != 3:
            raise ValueError('expected 2D or 3D input (got {}D input)'
                             .format(input.dim()))
