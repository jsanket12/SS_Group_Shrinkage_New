...
from GHS_resnet_models_non_center import SS_GHS_Node_ResNet #, SS_GHS_Node_Wide_ResNet
from GHS_linear_layers_non_center import GHS_layer
from GHS_Conv_layers_non_center import SS_GHS_Node_Conv2d_layer
from GHS_BN_layers_non_center import SS_GHS_VB_BatchNorm2d

parser = argparse.ArgumentParser(description='Cifar10 ResNet')

# Basic Setting
parser.add_argument('--seed', default=1, type = int, help = 'set seed')
parser.add_argument('--results_path', default='./results_resnet/cifar10/SS_GHS/', type = str, help = 'base path for saving result')
parser.add_argument('--warm_start_path', default='./', type = str, help = 'base path for saving result')
...
def main():
    ...
    net = SS_GHS_Node_ResNet(depth=args.depth, num_classes=target_dim, temp = temp, gamma_prior = gamma_prior, sigma_0 = sigma_0, tau_0 = tau_0, tau_1 = tau_1).to(device)
    ...
    # scheduler = torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=args.init_lr/args.lr_decay_factor, max_lr=args.init_lr)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            kl_train = 0.
            # kl_sig_total_contri = 0.
            for module in net.modules():
                if isinstance(module, (SS_GHS_Node_Conv2d_layer,SS_GHS_VB_BatchNorm2d,GHS_layer)):
    ...
    # m.save(PATH,'results_SS_Group_Horseshoe_Node_Resnet_CIFAR10_silu_lr_decay_'+str(args.batch_train)) 

    # torch.save(net.state_dict(), PATH + 'SS_Group_Horseshoe_Node_Resnet_CIFAR10_model_silu_lr_decay_'+ str(args.batch_train) + '.pt')
    ...
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,'SS_Group_Horseshoe_Node_Resnet_CIFAR10_loss_silu_lr_decay_'+str(args.batch_train)+'.png'),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,'SS_Group_Horseshoe_Node_Resnet_CIFAR10_accuracy_silu_lr_decay_'+str(args.batch_train)+'.png'),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,'SS_Group_Horseshoe_Node_Resnet_CIFAR10_sparsity_edge_silu_lr_decay_'+str(args.batch_train)+'.png'),dpi=300)
    # plt.close()
