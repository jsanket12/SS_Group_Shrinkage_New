...
from Group_Lasso_resnet_models_non_center import Group_Lasso_ResNet
from Group_Lasso_linear_layers_non_center import Group_Lasso_layer
from Group_Lasso_Conv_layers_non_center import Group_Lasso_Conv2d_layer
...
def main():
    ...
    net = Group_Lasso_ResNet(depth=args.depth, num_classes=target_dim, sigma_0 = sigma_0, 
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            for module in net.modules():
                if isinstance(module, (Group_Lasso_Conv2d_layer,Group_Lasso_VB_BatchNorm2d,Group_Lasso_layer)):
        ...
        with torch.no_grad():
            ...
            for name, module in net.named_modules():
                if isinstance(module, (Group_Lasso_Conv2d_layer)):
                    if 'downsample' not in name:
                        if module.v is not None:
                            param_overall += (module.in_channels*module.kernel_size[0]*module.kernel_size[1]+1)*module.out_channels
                            flops_overall += module.out_channels*(module.in_channels*module.kernel_size[0]*module.kernel_size[1]+1)* \
                            ...
                            param_overall += (module.in_channels*module.kernel_size[0]*module.kernel_size[1])*module.out_channels
                            flops_overall += module.out_channels*(module.in_channels*module.kernel_size[0]*module.kernel_size[1])* \
                                            (np.floor((module.input_size[2]+2*module.padding[0]-module.dilation[0]*(module.kernel_size[0]-1)-1)/module.stride[0]+1))**2/module.groups
            ...
            # sparsity_overall = 0
            # for module in net.modules():
            #     if isinstance(module, (Group_Lasso_Conv2d_layer,Group_Lasso_layer)):
            #         sparsity_overall += (module.w != 0).sum()
            #         if module.v is not None:
            #             sparsity_overall += (module.v != 0).sum()
            # sparsity_overall = sparsity_overall.div(total_num_para).item()

            flops_pruned_val = flops_overall
            param_pruned_val = param_overall
    ...
    m.save(PATH,'results_Group_Lasso_non_center_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)) 

    torch.save(net.state_dict(), PATH + 'Group_Lasso_non_center_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+ '_model.pt')

    plt.plot(range(num_epochs), train_Loss, 'b', label='Train')                
    plt.plot(range(num_epochs), test_Loss, 'orange', label='Test')
    plt.title('Group_Lasso: Train-Test Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    # plt.ylim([0.55, 1.0])
    plt.legend(loc='upper right', frameon=False)
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,'Group_Lasso_non_center_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_loss.png'),dpi=300)
    plt.close()

    plt.plot(range(num_epochs), train_Accuracy, 'b', label='Train')
    plt.plot(range(num_epochs), test_Accuracy, 'orange', label='Test')
    plt.title('Group_Lasso: Train-Test Accuracy')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    # plt.ylim([0.55, 1.0])
    plt.legend(loc='upper right', frameon=False)
    plt.grid(ls='dotted')
    plt.savefig(os.path.join(PATH,'Group_Lasso_non_center_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_sig0_'+str(args.sigma_0)+'_accuracy.png'),dpi=300)
    plt.close()
