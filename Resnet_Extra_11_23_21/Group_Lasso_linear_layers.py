...
##########################################################################################################################################
#### Spike-and-slab simultaneous node and edge selection with Group Lasso Layer
class SS_Group_Lasso_Node_and_Edge_layer(nn.Module):
    def __init__(self, input_dim, output_dim, lamb_mu, lamb_rho, bias=True,
        ...
        self.tau_star_mu = nn.Parameter(torch.Tensor(output_dim))
        self.tau_star_rho = nn.Parameter(torch.Tensor(output_dim))
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            w_tau_star_mu = self.tau_star_mu.expand(self.input_dim, self.output_dim)
            w_tau_star_sigma = sigma_tau_star.expand(self.input_dim, self.output_dim)
            ...
            kl_w = w_gamma_star * w_gamma * (torch.log(self.sigma_0) - torch.log(sigma_w) + 0.5*w_tau_star_mu - 0.5 + \
                    0.5*((sigma_w**2+self.w_mu**2)/self.sigma_0**2)*torch.exp(-w_tau_star_mu + (w_tau_star_sigma**2/2)))

            if self.v_mu is not None:
                v_gamma_star = gamma_star
                v_gamma = sigmoid(self.v_theta)
                v_tau_star_mu = self.tau_star_mu
                v_tau_star_sigma = sigma_tau_star
                ...
                kl_v = v_gamma_star * v_gamma * (torch.log(self.sigma_0) - torch.log(sigma_v) + 0.5*v_tau_star_mu - 0.5 + \
                        0.5*((sigma_v**2+self.v_mu**2)/self.sigma_0**2)*torch.exp(-v_tau_star_mu + (v_tau_star_sigma**2/2)))

                kl_tau = (0.5*self.input_dim+1)*(torch.log(torch.tensor(2))-self.lamb_mu-self.tau_star_mu) + torch.lgamma(torch.tensor(0.5*self.input_dim+1)) + \
                ...
                self.kl = torch.sum(kl_w_gamma) + torch.sum(kl_v_gamma) + torch.sum(kl_gamma_star)  + \
                            torch.sum(kl_w) + torch.sum(kl_v) + torch.sum(kl_tau)
            else:
                kl_tau = (0.5*self.input_dim+0.5)*(torch.log(torch.tensor(2))-self.lamb_mu-self.tau_star_mu) + torch.lgamma(torch.tensor(0.5*self.input_dim+0.5)) + \
                    0.5*torch.exp(self.lamb_mu + (lamb_sigma**2/2))*torch.exp(self.tau_star_mu + (sigma_tau_star**2/2)) -\
                    torch.log(sigma_tau_star) - 1.41894

                self.kl = torch.sum(kl_w_gamma) + torch.sum(kl_gamma_star) + torch.sum(kl_w) + torch.sum(kl_tau)

        return F.linear(input, self.w.T, self.v)

##########################################################################################################################################
#### Spike-and-slab node selection with Group Lasso Layer
class SS_Group_Lasso_Node_layer(nn.Module):
    def __init__(self, input_dim, output_dim, lamb_mu, lamb_rho,  bias=True,
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            kl_w = w_gamma * (torch.log(self.sigma_0) - torch.log(sigma_w) + 0.5*w_tau_star_mu - 0.5 + \
                    0.5*((sigma_w**2+self.w_mu**2)/self.sigma_0**2)*torch.exp(-w_tau_star_mu + (w_tau_star_sigma**2/2)))
            ...
            if self.v_mu is not None:
                ...
                kl_v = v_gamma * (torch.log(self.sigma_0) - torch.log(sigma_v) + 0.5*v_tau_star_mu - 0.5 + \
                        0.5*((sigma_v**2+self.v_mu**2)/self.sigma_0**2)*torch.exp(-v_tau_star_mu + (v_tau_star_sigma**2/2)))
                ...
                self.kl = torch.sum(kl_gamma) + torch.sum(kl_w) + torch.sum(kl_tau)

        return F.linear(input, self.w.T, self.v)

##########################################################################################################################################
#### Spike-and-slab edge selection with Group Lasso Layer
class SS_Group_Lasso_Edge_layer(nn.Module):
    def __init__(self, input_dim, output_dim, lamb_mu, lamb_rho, bias=True,
                    temp = 0.5, gamma_prior = 0.0001, sigma_0 = 1):
        super().__init__()
        # set input and output dimensions
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            if self.v_mu is not None:
                ...
                self.kl = torch.sum(kl_w_gamma) + torch.sum(kl_w) + torch.sum(kl_tau)

        return F.linear(input, self.w.T, self.v)

##########################################################################################################################################
#### Group Lasso without spike-and-slab Layer
class Group_Lasso_layer(nn.Module):
    def __init__(self, input_dim, output_dim, lamb_mu, lamb_rho, bias=True, sigma_0=1):
        super().__init__()
        # set input and output dimensions
        self.input_dim = input_dim
        self.output_dim = output_dim
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            kl_w = torch.log(self.sigma_0) - torch.log(sigma_w) + 0.5*w_tau_star_mu - 0.5 + \
                    0.5*((sigma_w**2+self.w_mu**2)/self.sigma_0**2)*torch.exp(-w_tau_star_mu + (w_tau_star_sigma**2/2))
            ...
            if self.v_mu is not None:
                ...
                kl_v = torch.log(self.sigma_0) - torch.log(sigma_v) + 0.5*v_tau_star_mu - 0.5 + \
                        0.5*((sigma_v**2+self.v_mu**2)/self.sigma_0**2)*torch.exp(-v_tau_star_mu + (v_tau_star_sigma**2/2))
