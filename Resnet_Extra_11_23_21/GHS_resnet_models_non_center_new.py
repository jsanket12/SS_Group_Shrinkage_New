...
from GHS_Conv_layers_non_center_new import SS_GHS_Node_Conv2d_layer, GHS_Conv2d_layer
from GHS_BN_layers_non_center import GHS_VB_BatchNorm2d
...
def GHS_conv3x3(in_planes, out_planes, stride=1, sigma_0 = 1):
    "3x3 convolution with padding"
...
class GHS_BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, sigma_0 = 1):
        super().__init__()
        self.conv1 = GHS_conv3x3(inplanes, planes, stride = stride, sigma_0 = sigma_0)
        self.bn1 = GHS_VB_BatchNorm2d(planes, sigma_0=sigma_0)
        self.conv2 = GHS_conv3x3(planes, planes, sigma_0=sigma_0)
        self.bn2 = GHS_VB_BatchNorm2d(planes, sigma_0=sigma_0)
        self.downsample = downsample
        self.stride = stride
...
class GHS_ResNet(nn.Module):
    ...
    def __init__(self, depth, num_classes=1000, sigma_0 = 1, tau_0 = 1e-5):
        ...
        self.inplanes = 16
        self.conv1 = GHS_Conv2d_layer(3, 16, kernel_size=3, padding=1, bias=False, sigma_0=sigma_0)
        self.bn1 = GHS_VB_BatchNorm2d(16, sigma_0=sigma_0)
        self.layer1 = self._make_layer(block, 16, n,)
        self.layer2 = self._make_layer(block, 32, n, stride=2)
        self.layer3 = self._make_layer(block, 64, n, stride=2)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                GHS_Conv2d_layer(self.inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=False, sigma_0=self.sigma_0),
                GHS_VB_BatchNorm2d(planes * block.expansion, sigma_0=self.sigma_0),
            )
