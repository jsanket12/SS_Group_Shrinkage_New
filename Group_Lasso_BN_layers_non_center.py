...
class SS_Group_Lasso_VB_BatchNorm(SS_Group_Lasso_VB_NormBase):
    ...
    def forward(self, input):
        ...
        if self.affine:
            ...
            self.weight = self.z * GL_scale *(self.weight_mu + weight_sigma * weight_epsilon)

            bias_sigma = torch.log1p(torch.exp(self.bias_rho))
            bias_epsilon = torch.zeros_like(self.bias_mu).normal_()
            self.bias = self.z * GL_scale *(self.bias_mu + bias_sigma * bias_epsilon)
...
class Group_Lasso_VB_BatchNorm(Group_Lasso_VB_NormBase):
    ...
    def forward(self, input):
        ...
        if self.affine:
            ...
            weight_sigma = torch.log1p(torch.exp(self.weight_rho))    
            weight_epsilon = torch.zeros_like(self.weight_mu).normal_()
            self.weight = GL_scale *(self.weight_mu + weight_sigma * weight_epsilon)

            bias_sigma = torch.log1p(torch.exp(self.bias_rho))
            bias_epsilon = torch.zeros_like(self.bias_mu).normal_()
            self.bias = GL_scale *(self.bias_mu + bias_sigma * bias_epsilon)
