...
from Gauss_linear_layers import Gauss_layer #, SSGauss_Edge_layer
from Gauss_Conv_layers_new import SSGauss_Node_Conv2d_layer , Gauss_Conv2d_layer #, SSGauss_Edge_Conv2d_layer
from Gauss_BN_layers import Gauss_VB_BatchNorm2d #, SSGauss_VB_BatchNorm2d
...
def SSGauss_Node_conv3x3(in_planes, out_planes, fine_tune, stride=1, temp = 0.5, gamma_prior = 0.0001, sigma_0 = 1):
    "3x3 convolution with padding"
    return SSGauss_Node_Conv2d_layer(in_planes, out_planes, fine_tune, kernel_size=3, stride=stride,
                     padding=1, bias=False, temp=temp, gamma_prior=gamma_prior, sigma_0=sigma_0)

class SSGauss_Node_BasicBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, fine_tune, stride=1, downsample=None, temp = 0.5, gamma_prior = 0.0001, sigma_0 = 1):
        super().__init__()
        self.conv1 = SSGauss_Node_conv3x3(inplanes, planes, fine_tune, stride, temp=temp, gamma_prior=gamma_prior, sigma_0=sigma_0)
        self.bn1 = Gauss_VB_BatchNorm2d(planes, sigma_0=sigma_0)
        self.conv2 = SSGauss_Node_conv3x3(planes, planes, fine_tune, temp=temp, gamma_prior=gamma_prior, sigma_0=sigma_0)
...
class SSGauss_Node_ResNet(nn.Module):
    def __init__(self, depth, num_classes=1000, temp = 0.5, gamma_prior = 0.0001, sigma_0 = 1):
        ...
        self.conv1 = SSGauss_Node_Conv2d_layer(3, 16,  fine_tune = self.fine_tune, kernel_size=3, padding=1, 
                                                bias=False, temp=temp, gamma_prior=gamma_prior, sigma_0=sigma_0)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                SSGauss_Node_Conv2d_layer(self.inplanes, planes * block.expansion, self.fine_tune,
        ...
        layers = []
        layers.append(block(self.inplanes, planes, self.fine_tune, stride, downsample, temp=self.temp,
                            gamma_prior=self.gamma_prior, sigma_0=self.sigma_0))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, self.fine_tune, temp=self.temp, gamma_prior=self.gamma_prior,
                                sigma_0=self.sigma_0))
