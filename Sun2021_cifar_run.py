...
import Sun2021_resnet
from Sun2021_conv_layer import Conv2d

parser = argparse.ArgumentParser(description='Cifar10 ResNet Compression')

# Basic Setting
parser.add_argument('--seed', default=1, type = int, help = 'set seed')
parser.add_argument('--base_path', default='./Sun_et_al_2021/result/cifar/sg_mcmc/', type = str, help = 'base path for saving result')
...
parser.add_argument('--prune_ratio', default = 0.1, type = float, help = 'prune ratio')
...
def main():
    ...
    net = Sun2021_resnet.ResNet_sparse(args.depth, args.num_classes).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            sparsity_path[epoch] = non_zero_element.item() / total_num_para_in
                ...
                if 'downsample' not in name:
                    if isinstance(module, (Conv2d)): 
                        if module.transposed:
                            conv_w = module.weight.permute(1, 0, 2, 3)   
                        ...
                        if module.bias is not None:
                            arr1_l = torch.norm(torch.cat((conv_w,module.bias.expand(1, module.bias.size()[0])),0),1,0)
                            out_prune_channels = torch.sum((arr1_l!=0).float())
                            param_overall += (module.weight != 0).sum() + (module.bias != 0).sum()
                            ...
                            out_prune_channels = torch.sum((arr1_l!=0).float())
                            param_overall += (module.weight != 0).sum()
    ...
    for epoch in range(num_epochs_fine_tune):
        ...
        with torch.no_grad():
            ...
            for name, para in net.named_parameters():
                total_num_para_overall += para.numel()
                non_zero_element_overall += (para.abs() > threshold).sum()
                if 'downsample' not in name and 'bn' not in name:
                    total_num_para_in += para.numel()
                    non_zero_element += (para.abs() > threshold).sum()
            print('sparsity overall:', non_zero_element_overall.item() / total_num_para_overall)
            ...
            print('sparsity:', non_zero_element.item() / total_num_para_in)
            print('Total number of parameters:', total_num_para_in)
            sparsity_path_fine_tune[epoch] = non_zero_element.item() / total_num_para_in
    ...
    m.save(args.base_path + args.model_path,'results_Sun2021_RN'+str(args.depth)+'_SGD_batch_'+str(args.batch_train)+'_prune_'+str(args.prune_ratio)+'_seed_'+str(args.seed))
